  ...
  if (!isConnected) {
    return (
      <div className="container py-10">
        ...
        <Link className="text-primary" href="/openclaw/connect">Перейти к подключению</Link>
      </div>
    )
  }
    ...
    <div className="container py-10 space-y-6">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          <Link className="text-primary" href="/openclaw/predictive">Predictive</Link>
          <Link className="text-primary" href="/openclaw/autoscale">AutoScale</Link>
          <Link className="text-primary" href="/openclaw/cost">Cost</Link>
        </div>
      </div>
            ...
            {agents.map(a => (
              <li key={a.id} className="flex items-center justify-between gap-3 border border-slate-800 rounded-lg p-3">
                ...
                <div className="flex gap-2">
                  <Link className="text-primary" href={`/openclaw/autoscale?agent=${encodeURIComponent(a.id)}`}>AutoScale</Link>
                </div>
              </li>
            ))}
          </ul>
